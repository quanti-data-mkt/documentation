...
A subdomain cloaked by a CNAME towards someone else's domain is recognisable as
such — uBlock Origin uncloaks CNAMEs on Firefox — and it makes your measurement
depend on a name you do not own. Records pointing straight at IP addresses keep
the whole chain inside your own domain.

Several providers only offer a CNAME, because their infrastructure cannot
guarantee a fixed address. Ours can, and that is deliberate.
