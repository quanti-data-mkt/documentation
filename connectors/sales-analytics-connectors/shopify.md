...
* **Product**: Product catalog with descriptions, categories, and merchandising attributes. Contains core product information including title, handle (URL slug), vendor, product type, publication status, timestamps (created, updated, published), full HTML description, and tags for organization and filtering. Also includes a `metafields` column — a JSON array of custom product metafields, each entry shaped as `{"namespace": ..., "key": ..., "value": ..., "type": ...}`. For complex types, the value is itself a JSON-encoded string. Capped at the first 250 metafields per product (Shopify API limit). Essential for product performance analysis, catalog management, and custom attribute enrichment.
...
* **Metafields**: Product metafields are now natively included in the `product` table as a JSON array. For other resource types (orders, customers, variants), metafields can be added upon request
